...
| `Source/Core/Map` | partial | `DBuilder.Map`, `DBuilder.IO` | Core map elements exist. Map options, grid setup, selection groups, typed field and argument helpers, tag collections, sidedef part semantics, and static format constraints are ported. Remaining collection behavior remains. |
