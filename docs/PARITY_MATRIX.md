...
| `Source/Core/Data` | partial | `DBuilder.IO` | Resource manager and data-location metadata exist, including resource-list serialization, duplicate combination, display names, PK3 root texture and flat options, palette lookup, main colormap lookup, named colormap lookup, colormap texture-name enumeration, sprite name enumeration, root PK3 classic texture composition with nested-WAD PNAMES fallback, and cache invalidation on clear/dispose. Full UDB DataManager parity is incomplete. |
