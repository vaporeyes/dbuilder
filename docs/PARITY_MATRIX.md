...
| `Source/Core/Map` | partial | `DBuilder.Map`, `DBuilder.IO` | Core map elements exist. Map identity settings, selection groups, tag labels, drawing defaults, script-tab settings, external command settings, resource location settings, plugin settings, and sidedef part semantics are ported. Full grid options and remaining collection behavior remain. |
