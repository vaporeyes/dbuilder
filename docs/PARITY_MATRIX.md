...
| `Source/Core/Editing` | partial | `DBuilder.Editor`, `DBuilder.Map`, `DBuilder.IO` | Selected editing workflows, active-mode select all and invert selection, UI-independent things filter evaluation, and configured things filter selection exist. Full edit-mode lifecycle and custom things filter editing remain. |
