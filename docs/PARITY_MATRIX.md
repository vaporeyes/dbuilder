...
| `Source/Core/Config` | partial | `DBuilder.IO` | Basic configuration parsing exists, including build defaults, required archives, static limits, texture sets, matching texture filters, linedef activations, compiler lists, nodebuilder lists, and several type sections. Full inheritance, includes, resource texture sets, and remaining type sections remain. |
