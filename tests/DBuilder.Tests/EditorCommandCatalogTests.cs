...
public class EditorCommandCatalogTests
{
    ...
    [InlineData("window.moveselectionup", "Move Selection Up by Grid Size", "Moves the selection in Edit Selection mode up by the current grid size.")]
    [InlineData("window.moveselectiondown", "Move Selection Down by Grid Size", "Moves the selection in Edit Selection mode down by the current grid size.")]
    [InlineData("window.moveselectionleft", "Move Selection Left by Grid Size", "Moves the selection in Edit Selection mode left by the current grid size.")]
    [InlineData("window.moveselectionright", "Move Selection Right by Grid Size", "Moves the selection in Edit Selection mode right by the current grid size.")]
    ...
    [Theory]
    [InlineData("window.align-things-to-wall", "Align Things to Wall", "Aligns selected things to the nearest linedef.")]
    [InlineData("window.find-replace", "Find and Replace", "Finds vertices, linedefs, sectors or things with a specific property, selects them and optionally replaces them with a given setting.")]
    public void WindowEditUtilityCommandsMatchUdbActionSurface(string commandId, string title, string description)
    {
        var command = EditorCommandCatalog.Find(commandId);
    ...
    [InlineData("map3d.brightness-up", "Sector brightness up", "Increases the targeted or selected sector brightness level by 8.")]
    ...
    [InlineData("map3d.brightness-down", "Sector brightness down", "Decreases the targeted or selected sector brightness level by 8.")]
    public void VisualBrightnessStepCommandsMatchUdbActionSurface(string id, string title, string description)
    {
        ...
        Assert.True(command.AllowMouse);
        Assert.Equal(
            id is not ("map3d.brightness-up" or "map3d.brightness-down"),
            command.AllowScroll);
        Assert.True(command.Repeat);
    }
