...
public sealed class MapControlCommandTests
{
    ...
    public void LinedefRenderingUsesUdbColorPresetModel()
    {
        ...
        Assert.Contains("float move = dt * 3.2f * _moveSpeed;", body, StringComparison.Ordinal);
        Assert.Contains("double sens = 0.005 * _mouseSpeed / Settings.DefaultMouseSpeed;", body, StringComparison.Ordinal);
    }
    ...
    public void CameraMovementRebuildsScreenSpaceThingGeometry()
    {
        ...
        int zoomIndex = body.IndexOf("private void ZoomBy(double factor)", StringComparison.Ordinal);
        ...
        Assert.True(body.IndexOf("_geometryDirty = true;", zoomIndex, StringComparison.Ordinal) > zoomIndex);
    ...
    [InlineData("map3d.visual-paint-select", "BeginVisualPaintSelection()")]
    [InlineData("map3d.visualpaintselect", "BeginVisualPaintSelection()")]
    ...
    public void ClassicPaintSelectUsesHeldActionState()
    {
        ...
        Assert.Contains("\"map3d.visual-paint-select\" or", body, StringComparison.Ordinal);
        Assert.Contains("\"map3d.visualpaintselect\";", body, StringComparison.Ordinal);
        ...
        Assert.Contains("if (commandId is \"map3d.visual-paint-select\" or \"map3d.visualpaintselect\") EndVisualPaintSelection();", body, StringComparison.Ordinal);
        Assert.Contains("if (IsVisualPaintSelectionActive()) ApplyVisualPaintSelection();", body, StringComparison.Ordinal);
    }
    ...
    [InlineData("map3d.rotatethingclockwise", "RotateVisualTargets3D(_gameConfig?.DoomThingRotationAngles == true ? 45 : 5, 5)")]
    [InlineData("map3d.rotatethingcounterclockwise", "RotateVisualTargets3D(_gameConfig?.DoomThingRotationAngles == true ? -45 : -5, -5)")]
    [InlineData("map3d.pitchthingclockwise", "ChangeThingPitchTargets3D(-5)")]
    [InlineData("map3d.pitchthingcounterclockwise", "ChangeThingPitchTargets3D(5)")]
    [InlineData("map3d.rollthingclockwise", "ChangeThingRollTargets3D(-5)")]
    [InlineData("map3d.rollthingcounterclockwise", "ChangeThingRollTargets3D(5)")]
    ...
    public void VisualSlopeHandleCommandsUseSelectedSurfacesAndLineHandles()
    {
        ...
        int canResolveIndex = body.IndexOf("private static bool CanResolveVisualSlopeLineHandlePair(", StringComparison.Ordinal);
        ...
        Assert.True(canResolveIndex > nearestResultIndex);
