...
public class SettingsTests
{
    ...
    [Fact]
    public void AutosavePreferencesDefaultAndClampLikeUdb()
    {
        var settings = new Settings();

        Assert.True(settings.Autosave);
        Assert.Equal(5, settings.NormalizedAutosaveCount);
        Assert.Equal(5, settings.NormalizedAutosaveIntervalMinutes);
        Assert.Equal(Settings.MinAutosaveCount, new Settings { AutosaveCount = 0 }.NormalizedAutosaveCount);
        Assert.Equal(Settings.MaxAutosaveCount, new Settings { AutosaveCount = 99 }.NormalizedAutosaveCount);
        Assert.Equal(Settings.MinAutosaveIntervalMinutes, new Settings { AutosaveIntervalMinutes = 0 }.NormalizedAutosaveIntervalMinutes);
        Assert.Equal(Settings.MaxAutosaveIntervalMinutes, new Settings { AutosaveIntervalMinutes = 99 }.NormalizedAutosaveIntervalMinutes);
    }
    ...
    [Fact]
    public void EditNewSectorDefaultsDisabledLikeUdb()
    {
        Assert.False(new Settings().EditNewSector);
    }
    ...
    public void RenderQualityPreferencesDefaultLikeUdb()
    {
        ...
        Assert.Equal(100, settings.NormalizedMoveSpeed);
        Assert.Equal(100, new Settings { MoveSpeed = 20 }.NormalizedMoveSpeed);
        Assert.Equal(2000, new Settings { MoveSpeed = 90000 }.NormalizedMoveSpeed);
        Assert.Equal(100, Settings.AcceptMoveSpeedText("20"));
        Assert.Equal(2000, Settings.AcceptMoveSpeedText("90000"));
        Assert.Null(Settings.AcceptMoveSpeedText("bad"));
        Assert.Equal(100, settings.NormalizedMouseSpeed);
        Assert.Equal(100, new Settings { MouseSpeed = 20 }.NormalizedMouseSpeed);
        Assert.Equal(2000, new Settings { MouseSpeed = 90000 }.NormalizedMouseSpeed);
        ...
        Assert.Equal(0, Settings.AcceptStitchRangeText("-5"));
        Assert.Equal(25, Settings.AcceptStitchRangeText("25"));
        Assert.Null(Settings.AcceptStitchRangeText("bad"));
        Assert.Equal(20, settings.NormalizedHighlightRange);
        Assert.Equal(0, new Settings { HighlightRange = -5 }.NormalizedHighlightRange);
        Assert.Equal(25, new Settings { HighlightRange = 25 }.NormalizedHighlightRange);
        Assert.Equal(0, Settings.AcceptHighlightRangeText("-5"));
        Assert.Equal(25, Settings.AcceptHighlightRangeText("25"));
        Assert.Null(Settings.AcceptHighlightRangeText("bad"));
    ...
    public void SaveAndLoadRoundTrips()
    {
        ...
        try
        {
            ...
            Assert.Equal(9, loaded.AutosaveCount);
            ...
            Assert.Equal(500, loaded.MoveSpeed);
