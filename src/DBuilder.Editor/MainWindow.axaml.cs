    ...
    private void UpdateInfo()
    {
        ...
        if (sv + sl + ss + st == 0)
        {
            InfoText.Text = $"Map: {_map.Vertices.Count} vertices, {_map.Linedefs.Count} linedefs, {_map.Sectors.Count} sectors, {_map.Things.Count} things." +
                            $"   Config: {_configName}.   Mode: {MapView.CurrentEditMode} (1 verts, 2 lines, 3 sectors, 4 things).   Click select; left-drag box-selects (or moves a grabbed vertex/thing); right-drag pans; wheel or -/= zoom; R fit; double-click edit; right-click splits; S/T toggle fills/things; Y sprites/arrows; D draw sector (Shift+D lines); I insert vertex/thing; M make sector at cursor; F flip linedef (Shift+F sidedefs); A align textures X (Shift+A Y); Ctrl/Cmd+C/V copy/paste; G snap, [ ] grid size; Delete removes (undoable).   Tab = 3D (WASD move, arrows or drag to look, QE up/down, G walk; wheel or right-drag raises/lowers targeted floor/ceiling/thing, Shift = 1; [ ] brightness; C/V copy/apply texture; T browse textures; A/Shift+A align; Shift+arrows nudge offset; O reset offsets; Enter edit properties; Delete removes a targeted thing).";
            return;
        }
